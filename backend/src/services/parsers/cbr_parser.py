...
class CBRParser:
    ...
    async def get_curr(date_req: date) -> list[CurrencyDTO]:
        ...
        async with aiohttp.ClientSession() as session:
            page_text = await fetch_url(url='https://cbr.ru/scripts/XML_daily.asp',
                                   session=session,
                                   queries={'date_req': date_req.strftime('%d/%m/%Y')})

        data = ET.fromstring(page_text)

        currencies = [CurrencyDTO(site='cbr.ru',
                                  cdr_id=None,
                                  nominal=1,
                                  char_code='RUB',
                                  value=1,
    ...
    @staticmethod
    async def get_curr_dynamic(curr_code: str,
                               start_date: date,
                               finish_date: date):

        async with aiohttp.ClientSession() as session:
            page_data = await fetch_url(f'http://www.cbr.ru/scripts/XML_dynamic.asp?date_req1={start_date.strftime('%d/%m/%Y')}&date_req2={finish_date.strftime('%d/%m/%Y')}&VAL_NM_RQ={curr_code}', session)
        ...
        for item in data:
            result.append(CurrencyDynamicPoint(
